...
public class InBuilder implements ConditionBuilder {
    ...
    public <T> Predicate build(SearchFilter filter, CriteriaBuilder criteriaBuilder, Root<T> root,
        ...
        if (fieldValue.contains(SearchConstant.FIELD_VALUE_SEPARATOR)) {
            List<Object> values = builderHelper.convertValues(fieldType,
                    fieldValue.split(SearchConstant.FIELD_VALUE_SEPARATOR));
            return path.in(values);
        }

        /*converting to equal condition if single value*/
