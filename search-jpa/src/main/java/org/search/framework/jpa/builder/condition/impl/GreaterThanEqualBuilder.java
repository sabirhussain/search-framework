...
public class GreaterThanEqualBuilder implements ConditionBuilder {
    ...
    public <T> Predicate build(SearchFilter filter, CriteriaBuilder criteriaBuilder, Root<T> root,
        ...
        if (fieldType == DataType.INTEGER || fieldType == DataType.DECIMAL) {
            return criteriaBuilder.ge(path.as(Number.class), builderHelper.convertToNum(fieldType, fieldValue));
